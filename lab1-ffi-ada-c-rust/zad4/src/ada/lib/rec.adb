...
package body Rec is
    ...
    function C_Rec_Factorial(N_Arg: in short) return C_Opt_I64 is
        Input  : Short_Integer := Short_Integer(N_Arg);
        Output : Opt_I64 := Rec_Factorial(Input);
        Result : C_Opt_I64;
    begin
        Result := (Value => long(Output.Value), Is_Present => Output.Is_Present);
        return Result;
    end C_Rec_Factorial;

    function C_Rec_Gcd(A_Arg: in int; B_Arg: in int) return C_Opt_I32 is
        Input_A  : Integer := Integer(A_Arg);
        Input_B  : Integer := Integer(B_Arg);
        Output : Opt_I32 := Rec_Gcd(Input_A, Input_B);
        Result : C_Opt_I32;
    begin
        Result := (Value => int(Output.Value), Is_Present => Output.Is_Present);
        return Result;
    end C_Rec_Gcd;

    procedure C_Rec_Solve(C_Eq: in C_Diophantine_Eq; C_Sol: in out C_Diophantine_Sol) is
        Input : Diophantine_Eq := (x_k => Integer(C_Eq.x_k), y_k => Integer(C_Eq.y_k), c => Integer(C_Eq.c));
        Output : Diophantine_Sol;
    begin
        Rec_Dio_Solve(Input, Output);
